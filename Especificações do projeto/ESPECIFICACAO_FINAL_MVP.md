...
> Plataforma de auditoria interna assistida por IA, em **Python full-stack (Reflex)** sobre **Google Cloud Platform** (Gemini, BigQuery, Cloud Storage, Firestore, Vertex AI Vector Search).
>
...
| # | Tema | Decisão |
|---|------|---------|
| **M1** | Backend | **Reflex puro** — sem FastAPI separado. Lógica em camada de serviços chamada pelos States. |
...
┌──────────────────────────────────────────────────────────────┐
│                      Reflex App (Python)                       │
│   Páginas/Componentes (UI)  ◄──►  States (event handlers)      │
│                                    │                            │
│                                    ▼                            │
│                          Camada de Serviços                     │
│  gemini/*   bigquery   firestore   storage   vector_search   rss│
└───┬─────────────┬──────────┬──────────┬───────────┬────────────┘
    ▼             ▼          ▼          ▼           ▼
 ┌──────┐  ┌──────────┐ ┌─────────┐ ┌────────┐ ┌──────────────┐
 │Gemini│  │ BigQuery │ │Firestore│ │ Cloud  │ │ Vertex AI    │
 │      │  │ (OLAP)   │ │ (OLTP)  │ │Storage │ │ Vector Search│
 └──────┘  └──────────┘ └─────────┘ └────────┘ └──────────────┘
        Auth: ADC / Service Account pessoal  ·  Budget alert ativo
```

**Stack:** Reflex `0.9.5` · Python `3.11` · `google-genai` · `google-cloud-bigquery` · `google-cloud-firestore` · `google-cloud-storage` · `google-cloud-aiplatform` (Vertex) · `feedparser` (RSS) · `pydantic-settings`.
...
## 8. Estrutura do projeto (Reflex)

```
clai/
├── clai/
│   ├── clai.py                 # rx.App + páginas/rotas
│   ├── core/
│   │   ├── config.py           # pydantic-settings (.env)
│   │   └── constants.py
│   ├── models/                 # schemas Pydantic das entidades
│   ├── services/
│   │   ├── gcp/
│   │   │   ├── auth.py
│   │   │   ├── bigquery_client.py
...
│   │   └── rss.py
│   ├── repositories/
│   ├── state/                  # 1 State por módulo
│   ├── pages/                  # 1 página por submódulo
│   └── components/
├── scripts/
...
3. `reflex init` e montar a estrutura da seção 8.
4. Implementar por ordem de prioridade (sugestão: **Planejamento → Abertura → Campo → Fechamento → FUP**).
